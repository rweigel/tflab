...
for N = [9,10,99,100,999,1000];
    ...
    logmsg(dbstack,'Test 1; N = %4d\n',N);
        Ep = zpredict(Z,B);
        mae = max(abs(Ep - Z(1)));
        assert(mae <= eps);
        logmsg(dbstack,'  max abs(error) = %.1e\n',mae);
        
    logmsg(dbstack,'Test 2 N = %4d\n',N);
    ...
    logmsg(dbstack,'Test 4; N = %4d\n',N);
        t = [0:N-1]';
        B = sin(2*pi*t/T);
        Z = abs(fft(B))/(N/2);
        Ep = zpredict(Z,B);
        mae = max(abs(Ep - B));
        logmsg(dbstack,'  T = %d; max abs(error) = %.1e\n',T,mae);
        % TODO: Document why mae is expected to scale with N.
        assert(mae < 10*N*eps);

    logmsg(dbstack,'Test 5; N = %4d\n',N);
