function [Z,stats] = robust_v1(ftB,ftE,opts)

    % If anything is modified that changes result or API, create a new
    % function with a different version number (e.g., robust_v1 ->
    % robust_v2.
    ...
    % and there are 2N weights. However, the code below computes N weights
    % based on the residual of abs(ftE - ftE_regression).
    ...
    Z(1,:) = regress(ftE,ftB); % Intial estimate.
    
    step = 2;
    laststep = 0;
    so = std(abs(ftE)); % Standard deviation of |output variable|.
    ...
    while 1
        ...
        % Robust estimate of standard deviation using median absolute
        % residual (Equation 7.2 of Huber 1981) who notes the motivation is
        % Andrews 1972. DuMouchel and O'Brien 1989 also cite Andrews 1972
        % as a motivation. The factor of 0.6745 makes it unbiased when when
        % residuals are gaussian. Excludes the smallest p values as
        % recommended by Hill and Holland 1977, "Two Robust Alternatives to
        % Least-Squares Regression", which was cited by DuMouchel and
        % O'Brien 1989. However, instead of using a factor of 2.1 as in
        % Hill and Holland 1977, we use a factor of 1.483 = 1/0.6745.
        ...
        if so/s > opts.snstop && laststep == 0
            if opts.verbose
                fprintf('robust_v1(): Stopping at step %d because std(abs(ftE))/mad(abs(residuals)) > 1000\n',step);
            end
            if step == 2
        ...
        Z(step,:) = regress(ftE.*sqrt(W),ftB.*repmat(sqrt(W),1,p));
        
        if laststep
            if opts.verbose
                fprintf('robust_v1(): Step %d: Hard cut-off last-step zeroed %d values (%.2f%%)\n',step,sum(Iz),100*sum(Iz)/length(R));
            end                
            break;
        end
        ...
        if step > 2
            ...
            if opts.verbose
                fprintf('robust_v1(): Step %d: ratio = abs(Z(step,:)-Z(step-1,:))./abs(Z(step-1,:)) = %.2g\n',step,ratio);
            end
            stop = any(ratio < opts.zeps);
            if step == opts.stepmax || stop == 1
                if stop && opts.verbose
                    fprintf('robust_v1(): Step %d: Last step because any(ratio) < opts.zeps\n',step);
                end
                if step == opts.stepmax && opts.verbose
                    fprintf('robust_v1(): Step %d: Last step because step == opts.stepmax = %d\n',step,opts.stepmax);
                end
